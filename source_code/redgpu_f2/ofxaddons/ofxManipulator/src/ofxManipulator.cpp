...
,m_manipulatorIsEnabled   (1)
,m_manipulatorIsFreezed   (0)
...
{
  if (m_manipulatorIsEnabled == 0) { return; }
  ...
  switch (m_currManipulator)
  {
    case (MANIPULATOR_SCALE):
    {
      ...
      drawTriangle(origin, screenf_half, ( (curr == SCALE_XZ) || (curr == SCALE_XYZ) ), axe_z, axe_x, unit_color_y);
    ...
    case (MANIPULATOR_TRANSLATION):
    {
      ...
      drawQuad(origin, screenf_half, ( (curr == TRANSLATION_XZ) || (curr == TRANSLATION_XYZ) ), axe_z, axe_x, unit_color_y);
...
{
  if (m_manipulatorIsEnabled == 0 || m_manipulatorIsFreezed == 1) { return; }
...
int ofxManipulator::isEnabled()
{
  return m_manipulatorIsEnabled;
}

void ofxManipulator::enable()
{
  m_manipulatorIsEnabled = 1;
}

void ofxManipulator::disable()
{
  m_manipulatorIsEnabled = 0;
}

int ofxManipulator::isFreezed()
{
  return m_manipulatorIsFreezed;
}

void ofxManipulator::freeze()
{
  m_manipulatorIsFreezed = 1;
}

void ofxManipulator::unfreeze()
{
  m_manipulatorIsFreezed = 0;
}

void ofxManipulator::getManipulatorState(
  int * outCurrScale,
...
{
  if (outCurrScale              != 0) { outCurrScale[0]              = (int)m_currScale;              }
  if (outCurrScalePredict       != 0) { outCurrScalePredict[0]       = (int)m_currScalePredict;       }
  if (outCurrRotation           != 0) { outCurrRotation[0]           = (int)m_currRotation;           }
  if (outCurrRotationPredict    != 0) { outCurrRotationPredict[0]    = (int)m_currRotationPredict;    }
  if (outCurrTranslation        != 0) { outCurrTranslation[0]        = (int)m_currTranslation;        }
  if (outCurrTranslationPredict != 0) { outCurrTranslationPredict[0] = (int)m_currTranslationPredict; }
}
  ...
  switch (m_currManipulator)
  {
    ...
    case (MANIPULATOR_TRANSLATION):
    {
      ...
      if (m_currTranslation == TRANSLATION_XYZ)
      {
        ...
        cam.setGlobalOrientation(m_viewInverse.getRotate());
        cam.setGlobalPosition(m_viewInverse.getTranslation());
